...
void main() {
  ...
  else /*if (DataIn.tex == 4)*/
	  color = texture(diffuseBl, DataIn.texCoord);


  vec4 texRust = texture(rust, DataIn.texCoord);
  vec3 normal = texture(normalMap, DataIn.texCoord).xyz;
  normal = normal * 2 - 1;
  normal = normalize(normal);

  float noise = clamp(pow(perlinNoise(DataIn.pos.xyz+0.5) *1.5, 4) , 0, 1);
  ...
  vec3 n = normal;
	...
	colorOut = max(intensity * color + spec * (1-noise) + color * 0.5,0);
}
